...
class User(db.Model):
    ...
    def verify_totp(self, token):
        """Validate a TOTP token and consume its time step.

        Only the current step and its immediate neighbours are accepted, and a
        step is atomically marked as used so the same token cannot be replayed
        by an attacker who observed it (for example over a shoulder or in a
        proxy log) while it is still inside the validity window.
        """
        if not self.otp_secret or self.id is None:
            return False
        ...
        try:
            # A conditional UPDATE lets the database arbitrate concurrent
            # attempts: exactly one request can claim a given time step.
            ...
            current_app.logger.error(
                'Cannot record OTP usage for user {0}. DETAIL: {1}'.format(
                    self.username, e))
            return False
    ...
    def create_local_user(self):
        ...
        try:
            ...
            # commits, or a case-insensitive duplicate slipped through.
            db.session.rollback()
            return {
                'status': False,
            ...
            db.session.rollback()
            return {'status': False, 'msg': 'Could not create user'}
    ...
    def update_profile(self, enable_otp=None):
        ...
        if enable_otp is not None:
            user.otp_secret = ""
            # The replay guard is tied to the secret it was recorded for, so
